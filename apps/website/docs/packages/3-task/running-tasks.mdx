...
Tasks can be cancelled using a canceler reference:
...
const longTask = Task.create(async ({ canceler: taskCanceler }) => {
  // Setup what happens on cancel
  taskCanceler.current = () => {
    console.log('Task was cancelled');
  };
  
  await new Promise(resolve => setTimeout(resolve, 10000));
  return Task.ok('completed');
});

// Run with canceler
const resultPromise = Task.run(longTask, canceler);
