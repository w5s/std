...
describe('RandomValue', () => {
  describe('.of', () => {
    it('should return new random value', () => {
      expect(RandomValue.of(0)).toBe(0);
      expect(RandomValue.of(1)).toBe(1);
    });
    it.each([Number.NaN, -1, 2])('should throw invariant when invalid value : %s', (invalidValue) => {
      expect(() => RandomValue.of(invalidValue)).toThrow(`Random value should be between 0 and 1. Got ${invalidValue}`);
    });
  });
